...
class execution(luigi.Task):
    ...
    def run(self):
        ...
        print("Connector Name: ", connectorName)
        carol_instance = Carol(os.environ['CAROLTENANT'], appName, apiAuth, connector_id=connectorId)
...
if __name__ == '__main__':
    ok = luigi.build([runMe()], local_scheduler=True, workers=1)
    if(os.environ['LONGTASKID'] != "TBD"):
        if ok:
            task.add_log(log_message="Everything went well.", log_level="INFO")
        else:
            task.add_log(log_message="Execution Failed.", log_level="ERROR")
